    ...
    def train(self, dataset, learning_rate=0.01, batch_size=100):
        ...
        while not converged:
            for x, y in dataset.iterate_once(batch_size):
                ...
                converged = True
                if last:
                    loss_diff = abs(nn.as_scalar(loss)-nn.as_scalar(last))
                last = loss
...
class DigitClassificationModel(object):
    ...
    def train(self, dataset, learning_rate=0.5, batch_size=100):
        """
        Trains the model.
        """
        accuracy = 0
...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        Returns:
            A node with shape (batch_size x 5) containing predicted scores
                (also called logits)
        """
        hidden_state = nn.ReLU(nn.AddBias(nn.Linear(xs[0], self.w1), self.b1))
        for char in xs[1:]:
            hidden_state = nn.ReLU(nn.AddBias(nn.Add(nn.Linear(char, self.w2), nn.Linear(hidden_state, self.weights)), self.b2))
        output_layer = nn.AddBias(nn.Linear(hidden_state, self.w3), self.b3)
        return output_layer
    ...
    def train(self, dataset, learning_rate=0.02, batch_size=200):
        """
        Trains the model.
        """
        accuracy = 0
        while accuracy < 0.85:
            for xs, y in dataset.iterate_once(batch_size):
                ...
                gradients = nn.gradients(loss, [self.w1, self.b1, self.w2, self.b2, self.w3, self.b3, self.weights])
                loss = nn.as_scalar(loss)
                self.w1.update(gradients[0], -learning_rate)
                self.b1.update(gradients[1], -learning_rate)
                self.w2.update(gradients[2], -learning_rate)
                ...
                self.weights.update(gradients[6], -learning_rate)
            accuracy = dataset.get_validation_accuracy()
