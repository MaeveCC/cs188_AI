...
def joinFactors(factors: List[Factor]):
    ...
    are the product of the corresponding rows of the input factors.
    ...
    variablesDomainDict = factorsList[0].variableDomainsDict()
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        newFactor = Factor([var for var in unconditioned_variables if var != eliminationVariable],
                           conditioned_variables, variablesDomainDict)
        assignments = newFactor.getAllPossibleAssignmentDicts()

        for assignment in assignments:
            probability = 0.0
            for eliminated_value in variablesDomainDict[eliminationVariable]:
                assignmentCopy = assignment.copy()
                assignmentCopy[eliminationVariable] = eliminated_value
                probability += factor.getProbability(assignmentCopy)
            newFactor.setProbability(assignment, probability)
        return newFactor
        "*** END YOUR CODE HERE ***"
    return eliminate
