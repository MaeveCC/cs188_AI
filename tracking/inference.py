...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    variables = [GHOST0, PAC, GHOST1, OBS0, OBS1]
    ...
    variableDomainsDict[OBS0] = list(range(X_RANGE + Y_RANGE + 6))
    variableDomainsDict[OBS1] = list(range(X_RANGE + Y_RANGE + 6))
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        allFactors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for eliminationVariable in eliminationOrder:
            unjoinedFactors, joinedFactors = joinFactorsByVariable(allFactors, eliminationVariable)
            allFactors = unjoinedFactors
            
            if len(joinedFactors.unconditionedVariables()) != 1:
                allFactors.append(eliminate(joinedFactors, eliminationVariable))
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        self.normalize()
        random_number = random.uniform(0, self.total())
        cumulative_weight = 0
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for newPosition in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, newPosition)
            for oldPosition in self.allPositions:
                newBeliefs[oldPosition] += self.beliefs[newPosition] * newPosDist[oldPosition]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        num_particles_per_position = self.numParticles // len(self.legalPositions)

        for position in self.legalPositions:
            self.particles.extend([position] * num_particles_per_position)

        remaining_particles = self.numParticles % len(self.legalPositions)
        if remaining_particles > 0:
            self.particles.extend(self.legalPositions[:remaining_particles])
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()
        believeDist = DiscreteDistribution()
        for particle in self.particles:
            believeDist[particle] += self.getObservationProb(observation, pacmanPosition, particle, jailPosition)
        if believeDist.total() == 0:
            self.initializeUniformly(gameState)
        else:
            believeDist.normalize()
            self.particles = [believeDist.sample() for i in range(self.numParticles)]
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            if particle not in positionDist.keys():
